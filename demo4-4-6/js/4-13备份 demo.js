...
//        source.unByKey(key);
			vectorLayer.getSource().forEachFeature(function(feature) {
              var code = feature.get('噪声');
              console.log(code)
              function zaosheng(code){
              	if(code<50){
              		return poor;
              ...
              if (code) {
                feature.set('incomeLevel', zaosheng(code));
              }
            });
        }
      });
...
//						console.log(feature.getGeometry().get('type'))
			return feature;
		});
